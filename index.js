...
const { indioma } = require('./src/indioma')
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Salve Corno(A) @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const time = moment.tz('Brazil/BRT').format('DD/MM AAAA:mm:ss')
			const date = moment.tz('Brazil/BRT').format('DD,MM,AAAA')
			...
			mess = {
				...
				success: '✔️ Bem Sucedido ✔️',
                                levelon: '❬ ✔ ❭ *Nivel Ativado Com Sucesso*',
				leveloff: ' ❬ X ❭  *Nível Desativado Com Sucesso*',
				levelnoton: '❬ X ❭ Seu Nível*',
				levelnol: '*NÍVEL FRACO AINDA* 0 °-°',
				error: {
				...
				only: {
					...
					admin: '[❗] Este comando só pode ser usado por administrador do grupo! ❌',
        		...
        		const bisakah = ['Você','Não Pode']
		        const kapankah = ['Outro dia, '' Outra semana ',' Outro mês ',' Outro ano']
			const botNumber = client.user.jid
			const ownerNumber = ["553398091185@s.whatsapp.net"] // replace this with your number
...
case 'timer':
				...
				break
                case 'indioma':
		client.sendMessage(from, indioma(prefix, sender), text, {quoted: mek})
                break
               case 'virtex':
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'ytmp3':
					...
					anu = await fetchJson(`https://mhankbarbar.tech/api/yta?url=https://youtu.be/6l5V3BWDcMw&apiKey=ApiKeyInHere')=${args[0]}&apiKey=${BarBarKey}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = `❏ *Título* : ${anu.Título}\n❏ *Tamanho* : ${anu.filesize}\n\nEspera aí, Guys, o áudio está sendo enviado novamente...`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', filename: `${anu.Título}.mp3`, quoted: mek})
					break
				case 'ytmp4':
					if (args.length < 1) return reply('Cadê O Link ??')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
					anu = await fetchJson(`https://mhankbarbar.tech/api/ytv?url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = `*❏ Título* : ${anu.Título}\n\n*Vídeo Está Sendo Enviado, Aguarde!*`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.Título}.mp4`, quoted: mek})
					break
				case 'trendtwit':
					client.updatePresence(from, Presence.composing) 
                                        if (!isUser) return reply(mess.only.daftarB)
	...
	case 'kbbi':
	      ...
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (!isUser) return reply(mess.only.daftarB)
               if (data.error) return reply(data.error)
                 infomp3 = `*Música Encontrada!!!*\nTítulo : ${data.result.Título}\nAlbum : ${data.result.album}\nDipublikasi : ${data.result.dipublikasi}`
                buffer = await getBuffer(data.result.thumb)
                lagu = await getBuffer(data.result.mp3)
                client.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
                client.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${data.result.Título}.mp3`, quoted: mek})
                break
				case 'info':
					me = client.user
				...
				case 'tts':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Kode indiomanya mana om?', text, {quoted: mek})
                                   if (!isUser) return reply(mess.only.daftarB)
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'Já Vai Tarde👋', text) // ur cods
					}, 0)
                     break

				case 'chord':
					if (args.length < 1) return reply('Título lagunya mana kak')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)					
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('Cadê a porra da letra🤬?')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
					anu = await fetchJson(`https://arugaz.herokuapp.com/api/lirik?Título=${tels}`, {method: 'get'})
					reply(anu.result)
					break
			case 'igstalk':
	    ...
	    case 'kapankah':
			...
			case 'travar':
					...
					var nomor = mek.participant
					const close = {
					text: `Grupo fechado pelo administrador @${nomor.split("@s.whatsapp.net")[0]}\nAgora *Apenas admin* Pode Enviar Mensagem Fudeu`,
					contextInfo: { mentionedJid: [nomor] }
					}
                ...
                case 'destravar':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					open = {
					text: `Grupo aberto pelo administrador @${sender.split("@")[0]}\nAgora Todos Os Membros Pode Enviar Mensagem`,
					contextInfo: { mentionedJid: [sender] }
					}
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                ...
                client.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
                client.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.Título}.mp3`, quoted: mek})
                break
                     case 'infocuaca':
                   tels = body.slice(11)
				...
				if (isUser) return reply(mess.only.daftarB)
					...
					break
                                case 'nsfwneko':
				    try{
						if (!isNsfw) return reply('❌ *NSFW NÃO ESTÁ ATIVADO* ❌')
                                                if (!isUser) return reply(mess.only.daftarB)
				...
				case 'logowolf':
					...
					if (Number(args[0]) === 1) {
						...
						reply('NSFW ATIVADO PUNHETEIROS🌚')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
